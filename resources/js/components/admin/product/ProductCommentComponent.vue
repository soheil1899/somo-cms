<template>
    <div>
        <h4 class="admin-title py-3 px-4 mb-4"> کامنتهای مربوط به محصول
            {{product_faname}}
            <a :href="'../product'" class="back-btn btn btn-dark btn-sm mr-1">برگشت</a>
                    ...
                    <div class="card mb-2 ml-1 mr-2" v-if="childitem.parent_id == item.id">
                        <div class="card-header row m-0 px-3">
                            ...
                            <div class="col-6 p-0 icons" style="text-align: left">
                                ...
                                <i title="ویرایش" @click="edit(childitem.id, childitem.title, childitem.comment)"
    ...
    export default {
        ...
        methods: {
            ...
            reloadPage() {
                ...
                axios.post('/dashboard/getproductcomment', data)
                    .then(function (response) {
                        console.log(response.data);
                        that.product_faname = response.data[0][0]['faname'];
                        that.commentlist = response.data[1];
                        ...
                        that.commentchilds = response.data[2];
